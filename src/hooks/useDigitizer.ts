...
export function useDigitizer(onError?: (message: string) => void) {
  ...
  const [lines, setLines] = useState<Line[]>([
    { id: 1, name: 'Line 1', points: [], regression: null, color: '#ff0000' },
  ]);
  ...
  useEffect(() => {
    const storedImg = localStorage.getItem('digitizer-image');
    if (storedImg) {
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = () => {
        localStorage.setItem('digitizer-image', reader.result as string);
        const image = new Image();
        image.onload = () => {
          setImg(image);
          resetApp();
  ...
  const resetApp = () => {
    ...
    localStorage.removeItem('digitizer-image');
  };
  ...
  const addPoint = (x: number, y: number) => {
    ...
    setLines((prev) =>
      prev.map((l) =>
        l.id === currentLineId
          ? { ...l, points: [...l.points, newPoint], regression: null }
          : l
      )
    );
  };
  ...
  const calculateRegression = () => {
    ...
    setLines((prev) =>
      prev.map((l) => (l.id === currentLineId ? { ...l, regression: reg } : l))
    );
  };
  ...
  const exportCSV = () => {
    ...
    for (let p = 0; p < maxPoints; p++) {
      ...
      sortedLines.forEach((line) => {
        ...
        row.push(
          point ? point.val.x.toFixed(4) : '',
          point ? point.val.y.toFixed(4) : ''
        );
      });
      rows.push(row.join(','));
    }
  ...
  const addLine = () => {
    ...
    setLines((prev) => [
      ...prev,
      { id: newId, name: `Line ${newId}`, points: [], regression: null, color },
    ]);
    ...
    setLines(
      lines.map((line) =>
        line.id === currentLineId
          ? { ...line, points: line.points.filter((p) => p.id !== id) }
          : line
      )
    );
  };
